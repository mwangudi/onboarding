...
namespace OnBoarding.Controllers
{
    public class DigitalDeskController : Controller
    {
        ...
        public List<NotificationsViewModel> GetSystemNotificationsList(string searchMessage, string searchDate, int jtStartIndex, int jtPageSize, int count, string jtSorting)
        {
            ...
            using (var db = new DBModel())
            {
                IEnumerable<NotificationsViewModel> query = db.Database.SqlQuery<NotificationsViewModel>("SELECT n.Id, n.[Type], n.[To], n.[From], n.MessageBody, n.[Sent], n.Action, CONVERT(varchar, n.DateCreated, 120) DateCreated FROM Notifications n ORDER BY " + jtSorting + " OFFSET " + jtStartIndex + " ROWS FETCH NEXT " + jtPageSize + " ROWS ONLY;");

                //Search  
                if (!string.IsNullOrEmpty(searchMessage) && string.IsNullOrEmpty(searchDate))
                {
                    query = db.Database.SqlQuery<NotificationsViewModel>("SELECT n.Id, n.[Type], n.[To], n.[From], n.MessageBody, n.[Sent],  n.Action, CONVERT(varchar, n.DateCreated, 120) DateCreated FROM Notifications n WHERE n.[To] LIKE '%" + searchMessage + "%' ORDER BY n.Id DESC OFFSET " + jtStartIndex + " ROWS FETCH NEXT " + jtPageSize + " ROWS ONLY;");

                }
                else if (!string.IsNullOrEmpty(searchDate) && string.IsNullOrEmpty(searchMessage))
                {
                    query = db.Database.SqlQuery<NotificationsViewModel>("SELECT n.Id, n.[Type], n.[To], n.[From], n.MessageBody, n.[Sent],  n.Action, CONVERT(varchar, n.DateCreated, 120) DateCreated FROM Notifications n WHERE CAST(n.DateCreated AS date) = " + "'" + searchDate + "'" + " ORDER BY n.Id DESC OFFSET " + jtStartIndex + " ROWS FETCH NEXT " + jtPageSize + " ROWS ONLY;");
                }
                else if (!string.IsNullOrEmpty(searchMessage) && !string.IsNullOrEmpty(searchDate))
                {
                    query = db.Database.SqlQuery<NotificationsViewModel>("SELECT n.Id, n.[Type], n.[To], n.[From], n.MessageBody, n.[Sent],  n.Action, CONVERT(varchar, n.DateCreated, 120) DateCreated FROM Notifications n WHERE CAST(n.DateCreated AS date) = " + "'" + searchDate + "'" + " AND n.[To] LIKE '%" + searchMessage + "%' ORDER BY n.Id DESC OFFSET " + jtStartIndex + " ROWS FETCH NEXT " + jtPageSize + " ROWS ONLY;");
                }
                else
                {
                ...
                else if (jtSorting.Equals("Action ASC"))
                {
                    query = query.OrderBy(p => p.Action);
                }
                else if (jtSorting.Equals("Action DESC"))
                {
                    query = query.OrderByDescending(p => p.To);
                }
                else
        ...
        public PartialViewResult LoadResendNotification(int getNotificationId)
        {
            using (DBModel db = new DBModel())
            {
                ...
                //ViewData["MessageBody"] = Regex.Replace(inputHTML, @"<[^>]+>|&nbsp;", getMessageInfo.MessageBody).Trim();
            }
        ...
        public List<RegisteredClient> GetRegisteredClientsList(string searchMessage, string searchFromDate, string searchToDate, int jtStartIndex, int count, string jtSorting)
        {
            ...
            using (DBModel db = new DBModel())
            {
                IEnumerable<RegisteredClient> query = db.Database.SqlQuery<RegisteredClient>("SELECT * FROM RegisteredClients r WHERE r.Status <> 4;");
        ...
        public PartialViewResult LoadDeleteClient(int getClientId)
        {
            using (DBModel db = new DBModel())
            {
                ...
                //Data For View Display
                ViewData["CompanyName"] = getClientInfo.Surname + " " + getClientInfo.OtherNames;
        ...
        public ActionResult DeleteClient(DeleteClientViewModel model)
        {
            using (DBModel db = new DBModel())
            {
                try
                {
                    ...
                    newDeletedEntry.DeletedBy = UserEmail.Email;
                    ...
                    if (recordDeleted > 0)
                    {
                        ...
                        db.ClientCompanies.RemoveRange(db.ClientCompanies.Where(r => r.ClientId == model.ClientId));
                        db.SaveChanges();
